...
| [probe-combo-lines.sql](../supabase/verify/probe-combo-lines.sql) | `compose_combo_bet` (slip-shaped: compose=bet invariants, shuffled-order dedup, 9 validation negatives incl. duplicate-spec ticket, multi-combo parlay ×4, combo+line parlay ×4), combo anti-tank (self-under RAISE / non-member allowed), RSVP-out auto-void erasure (market+bets+ledger+feed gone, balances restored, no resurrection), `settle_week` c‴ on both clocks (strikes Σ, total_pins Σ, pending-exempt import-less combo → `void_missing` delete-refund, idempotent re-settle) |
