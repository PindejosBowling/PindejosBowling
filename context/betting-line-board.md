...
**Projection strip** — under the player select (hidden while a combo is armed), `BookProjectionCard` shows the selected player's **book projection vs season average** for pins / clean frames / strikes / spares: `betMarkets.playerProjection` → `odds_engine_player_projection` (per-game values, cached per player in `projCache`, reset with the board-reload effect), scope-scaled × the night's game count for Weekly, ▲/▼ delta vs the average, `*` footnote when the average fell back to lifetime/league. Self-hides when the engine is off (`projected` all null). Details: [odds-engine.md](odds-engine.md).
...
3. **Pivot** — the board swaps to a member-picking list: **every RSVP'd-in player** (`rsvpInPlayers` — RSVP is the compose RPC's only eligibility rule; a player with no individual line still combines), viewer first. Each row leads with a **big accent value** next to the `+`/`✓` `PickChip`: the player's solo line for the stat when one exists (`4.5+`, SOLO LINE caption), else the **book's projection** (BOOK caption — no empty hyphen; muted `—` only when neither exists, i.e. engine off with no market). Under each name: the scope-scaled average (`SEASON/LIFETIME/LEAGUE AVG …`, `comboMemberAverages`) and — on solo-line rows only (no duplicate where BOOK is already big) — `· BOOK …` with a ▲/▼ vs the average (`betMarkets.memberProjections` → `odds_engine_member_projections`, same per-game × games scaling; omitted when the engine is off). A ▲ member is one the book rates above their average — their share of the combo line prices richer than the averages suggest. Once the combo is quotable (2+ members, scope open), the **combo value card** renders ABOVE the member list: the tap-to-type value field (→ the same `LineEntrySheet`) + live odds + the `AVG … · BOOK …` group yardsticks — the value editor lives here, not in the BuilderBar. Above it (under the hint), the pivot heads with the **same `BookProjectionCard` the flat board uses**, summed across the chosen members — all four stats' group book expectation vs combined averages, fed from the screen's per-player `projCache` (missing members fetch on pick, deduped by `projRequested`), rendered once every picked member's rows are cached. The player select and specials/combo rows hide; **the scope pills stay live** — the combo's scope follows the board's scope filter (Weekly → `night` w/ `nGames = max(weekGameNumbers.length, 1)`, Game N → `game`), and a mid-build switch just re-previews.
4. **BuilderBar** — a floating bar in the slip bar's exact footprint (the provider's **`setSlipBarHidden`** flag yields it; restored by an effect cleanup on every exit). Left: member names + a compact `value+ · ×odds` price line (the value EDITOR moved to the combo value card above the member list — the bar just shows what Add will stage); pricing still runs through `useLinePreview({kind:'combo'})` → `combo_price_line` (the quote's `seed_line` anchors the value; `comboLineValue` state, null = seed, resets on combo-identity change); right: Cancel + **Add** (disabled until 2+ members and a priced value, or when the scope is in progress — the RPC would reject). The staging key is the canonical `stat|('night'|gameN)|sortedMemberIds` — when that exact combo is already staged the CTA flips to **Remove** (`stageCombo` toggles by key; a blind Add would silently unstage).
...
- **`BuilderBar`** (`{ memberNames, statLabel, scopeLabel, value, onEditValue, quote, quoteLoading?, minMembers, alreadyStaged, blocked?, onAdd, onCancel }`) — the combine-mode floating bar (slip-bar footprint); its value opens the same `LineEntrySheet`.
