...
- **`LinePill`** (`{ line, value, odds, onEditValue?, onStage?, staged?, dimmed?, inert?, editable? }`) — one market's value-first pill: the value (tap → `onEditValue`, which the screen wires to the `LineEntrySheet`) + the condition label (`PINS` / `TOTAL PINS` / stat label, exported `conditionLabel()`) + the price (`—` when unpriced). The pill BODY tap stages/unstages the displayed value; the value tap never stages. `dimmed` is cosmetic-only (still pressable → toasts); `editable=false` in armed combine mode (taps seed the combo).
