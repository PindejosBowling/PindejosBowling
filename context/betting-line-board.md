...
- **Every-game legs** (`game_number: null`) make the line **materialize once per game** on the week's schedule — each instance binds its null-game legs to that game ("the bettor bowls their over *in this game*" → one offering in each game's group). Instances get synthetic ids (`<rowId>:g<N>`) and flow through the normal per-game board placement; an instance whose game can't resolve (subject not in that game's lineup) is hidden individually. The create modal's game picker offers `G1/G2/G3/ALL`, and blocks an ALL leg overlapping a fixed-game leg for the same subject (the instance would double up on one market).
...
- **Board placement is derived.** All legs in one game → the special renders in that game's group (a `SPECIALS`-labeled card after the standard sections); legs across games — or a game group the viewer's board doesn't show — → the top-level `SPECIALS` section after the game groups. Rendered by **`CustomLineRow`** (`{ line, isLast, inProgress?, disabled?, onTake? }`): title/description/leg summary left, one `×odds TAKE` button right; `category === 'special'` gets the gold chip + colors.
- **Take flow.** TAKE opens a dedicated wager sheet (leg list, `PAYS ×odds`, to-win preview) → `bets.place(line.selectionIds, wager)`. Specials **never enter the parlay slip** (they already are a bundle), so TAKE behaves identically in Single and Parlay modes.
- **Bet branding is client-matched, best-effort.** The placed bet is unmarked in the DB. The hook decorates `BetView.customLineTitle/customLineCategory` when a bet's selection set exactly equals a currently-resolved special's bundle; `BetRow`/`BetDetailModal` then headline the title (gold for `special`). Self-referential lines resolve to different selections per taker, so the hook additionally re-resolves them **per bettor in view** to brand other players' bets. Once the underlying markets settle (or the line is deleted/re-scoped), the match lapses and the bet renders as a plain single/parlay — accepted trade-off for keeping the DB untouched. A hand-built parlay identical to a special also picks up its title (harmless).
