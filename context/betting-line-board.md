...
- **`LineEntrySheet`** (`{ title, conditionLabel, scopeLabel?, source, initialValue, onAccept(value, quote), onClose }`) — the shared value editor (board pills + BuilderBar), a `BottomSheet` with a `decimal-pad` input: shows the acceptable min–max band from the quote, re-prices the `snapToHalf`-snapped draft live via its own `useLinePreview(source, draft)` (market or combo source), and Accept (disabled until the draft is priced) returns `(value, quote)` to the caller. One instance per screen, conditional-mount (`{valueSheet && …}`) so state resets. `snapToHalf` (integers step UP: "140" → 140.5) is exported from here. ⚰️ The inline `LineStepper` (in-pill TextInput) is retired — the board never raises the keyboard now (the keyboard's accessory toolbar + coverage caused dead taps on lower pills).
