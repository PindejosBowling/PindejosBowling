...
2. **Subject selector** — the player `Dropdown`'s slot becomes a wrapped **member chip row** (`PickChip` per short name, multi-select): **every RSVP'd-in player** (`rsvpInPlayers` — RSVP is the compose RPC's only eligibility rule; a player with no individual line still combines), viewer first. **The scope pills stay live** — the combo's scope follows the board's scope filter (Weekly → `night` w/ `nGames = max(weekGameNumbers.length, 1)`, Game N → `game`), and a mid-build switch re-anchors values + re-quotes.
...
5. **Member context rows** — under the line card, one tinted row per pool member: name, scope-scaled Total Pins `SEASON/LIFETIME/LEAGUE AVG …` (`poolStats`) `· FORECAST …` with a ▲/▼ riding the BOOK vs the average (▲ = the book rates the member above their average — their share of a combo line prices richer than the averages suggest), and the `+`/`✓` `PickChip`. ⚰️ The big SOLO LINE value column is retired — solo lines are one toggle away on the normal board.
