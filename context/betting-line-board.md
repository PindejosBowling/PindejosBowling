...
5. **Add** — builds the `SlipCombo` spec, `stageCombo`s it, exits combine mode, and opens the slip (mirroring the old composer's add-then-open). Exit paths: Cancel, Add, view change / readOnly flip (effect reset), unmount.
