...
3. **Pivot** — the board swaps to a member-picking list: **every RSVP'd-in player** (`rsvpInPlayers` — RSVP is the compose RPC's only eligibility rule; a player with no individual line still combines), viewer first. Each row leads with the player's **solo line for the stat as a big accent value** (`4.5+`, with a small SOLO LINE caption — it's the number a combiner shops on; muted `—` when they have no individual market) next to the `+`/`✓` `PickChip`. The player select and specials/combo rows hide; **the scope pills stay live** — the combo's scope follows the board's scope filter (Weekly → `night` w/ `nGames = max(weekGameNumbers.length, 1)`, Game N → `game`), and a mid-build switch just re-previews.
