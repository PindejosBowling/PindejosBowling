...
> arrows nudge ±0.5 inside the priceable band, tapping the number opens the
> keyboard for direct entry, and the price follows the value live
...
4. **BuilderBar** — a floating bar in the slip bar's exact footprint (the provider's **`setSlipBarHidden`** flag yields it; restored by an effect cleanup on every exit). Left: member names + the same `◀ value ▶` editor the board pills use (`LineStepper`), priced live by `useLinePreview({kind:'combo'})` → `combo_price_line` (the quote's `seed_line` anchors the editor; `comboLineValue` state, null = seed, resets on combo-identity change); right: Cancel + **Add** (disabled until 2+ members and a priced value, or when the scope is in progress — the RPC would reject). The staging key is the canonical `stat|('night'|gameN)|sortedMemberIds` — when that exact combo is already staged the CTA flips to **Remove** (`stageCombo` toggles by key; a blind Add would silently unstage).
...
- **`LinePill`** (`{ line, value, odds, loading?, band?, onValueChange?, onStage?, staged?, dimmed?, inert?, editable? }`) — one market's value-first pill: `◀ value ▶` (the shared `LineStepper`) + the condition label (`PINS` / `TOTAL PINS` / stat label) + the live price (`…` while pricing, `—` + a "line unavailable" microcopy past the band). The pill BODY tap stages/unstages the displayed value; the stepper/type-in never stages. `dimmed` is cosmetic-only (still pressable → toasts); `editable=false` in armed combine mode (taps seed the combo).
- **`LineStepper`** (`{ value, onChange, min?, max?, disabled?, onFill?, suffix? }`) — the shared `◀ value ▶` cluster (board pills + BuilderBar): arrows nudge ±0.5 clamped to the quoted band; tapping the number opens `decimal-pad` input, committed on blur/submit (Android's pad has no reliable submit key) with `snapToHalf` (integers step UP: "140" → 140.5; garbage reverts).
