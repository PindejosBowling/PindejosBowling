...
- **`LineRow`** (`{ line, isLast, inProgress?, selectionState?, onSelect? }`) — presentational row for one market. Subject (+ optional `subtitle` metadata) on the left; **one pick button per `line.selections`** on the right (data-driven, never hardcoded over/under). **The button IS the line being agreed to** (`selectionButtonLabel`): every offered side is an over (unders are UI-hidden — all bets are over by definition), so an over side renders as its threshold — `142.5+` on a score line, `4.5+` on a stat prop — while lineless sides (moneyline) keep their label (`WIN`). Mirrors `BetRow`'s "callers gate the callbacks" design:
...
- **`LineView.statKey`** (from `params.stat`) + a `subtitle` naming the stat (`STRIKES`) — the threshold itself lives in the pick button (`4.5+`, via `selectionButtonLabel`).
