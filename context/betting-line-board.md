...
> `value CONDITION … ×odds` (`LinePill` + the shared `LineStepper`):
> tapping the number opens the keyboard for direct entry (the priceable
> min–max band shows under the input while typing; no arrows — tap-to-type
> only), and the price follows the value live
...
4. **BuilderBar** — a floating bar in the slip bar's exact footprint (the provider's **`setSlipBarHidden`** flag yields it; restored by an effect cleanup on every exit). Left: member names + the same tap-to-type value editor the board pills use (`LineStepper`), priced live by `useLinePreview({kind:'combo'})` → `combo_price_line` (the quote's `seed_line` anchors the editor; `comboLineValue` state, null = seed, resets on combo-identity change); right: Cancel + **Add** (disabled until 2+ members and a priced value, or when the scope is in progress — the RPC would reject). The staging key is the canonical `stat|('night'|gameN)|sortedMemberIds` — when that exact combo is already staged the CTA flips to **Remove** (`stageCombo` toggles by key; a blind Add would silently unstage).
...
- **`LinePill`** (`{ line, value, odds, loading?, band?, onValueChange?, onStage?, staged?, dimmed?, inert?, editable? }`) — one market's value-first pill: the tap-to-type value (the shared `LineStepper`) + the condition label (`PINS` / `TOTAL PINS` / stat label) + the live price (`…` while pricing, `—` + a "line unavailable" microcopy past the band). The pill BODY tap stages/unstages the displayed value; the stepper/type-in never stages. `dimmed` is cosmetic-only (still pressable → toasts); `editable=false` in armed combine mode (taps seed the combo).
- **`LineStepper`** (`{ value, onChange, min?, max?, disabled?, onFill?, suffix? }`) — the shared tap-to-type value editor (board pills + BuilderBar): tapping the number opens `decimal-pad` input with the quoted min–max band shown under it while typing, committed on blur/submit (Android's pad has no reliable submit key) with `snapToHalf` (integers step UP: "140" → 140.5; garbage reverts) clamped to the band. No arrows.
...
- **`BuilderBar`** (`{ memberNames, statLabel, scopeLabel, value, onValueChange, quote, quoteLoading?, minMembers, alreadyStaged, blocked?, onAdd, onCancel }`) — the combine-mode floating bar (slip-bar footprint) carrying the same `LineStepper` value editor.
