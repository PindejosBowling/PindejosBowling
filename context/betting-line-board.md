...
The board is driven by **two screen-local filters** (plain `useState`, reset on each visit):
...
- **Player select** — a `selectable` `Button` field opening `PlayerPickerModal` (searchable sheet, avatars), populated **only by players with in-scope availability**: their own visible lines ∪ combo membership ∪ special involvement. Names seed from `rsvpInPlayers` first so resolved-leg display names ("You") never leak into the picker.
...
- **`PlayerPickerModal`** (`{ visible, items, onSelectItem, onClose, title }`, id-aware mode) — the reused searchable player sheet backing the board's player filter; the trigger is a `selectable` `Button` field (value + chevron), suffixed `(you)` for the viewer.
