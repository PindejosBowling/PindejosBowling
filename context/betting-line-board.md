...
3. **Pivot** — the board swaps to a member-picking list: **every RSVP'd-in player** (`rsvpInPlayers` — RSVP is the compose RPC's only eligibility rule; a player with no individual line still combines), viewer first. Each row leads with the player's **solo line for the stat as a big accent value** (`4.5+`, with a small SOLO LINE caption — it's the number a combiner shops on; muted `—` when they have no individual market) next to the `+`/`✓` `PickChip`. Under each name: the scope-scaled average (`SEASON/LIFETIME/LEAGUE AVG …`, `comboMemberAverages`) **and the book's projection** (`· BOOK …` with a ▲/▼ vs the average — `betMarkets.memberProjections` → `odds_engine_member_projections`, same per-game × games scaling; segment omitted when the engine is off). A ▲ member is one the book rates above their average — their share of the combo line prices richer than the averages suggest. The player select and specials/combo rows hide; **the scope pills stay live** — the combo's scope follows the board's scope filter (Weekly → `night` w/ `nGames = max(weekGameNumbers.length, 1)`, Game N → `game`), and a mid-build switch just re-previews.
4. **BuilderBar** — a floating bar in the slip bar's exact footprint (the provider's **`setSlipBarHidden`** flag yields it; restored by an effect cleanup on every exit). Left: member names + the combo's value (tap → the same `LineEntrySheet` the board pills use, fed the `{kind:'combo'}` preview source), priced live by `useLinePreview({kind:'combo'})` → `combo_price_line` (the quote's `seed_line` anchors the value; `comboLineValue` state, null = seed, resets on combo-identity change); right: Cancel + **Add** (disabled until 2+ members and a priced value, or when the scope is in progress — the RPC would reject). The staging key is the canonical `stat|('night'|gameN)|sortedMemberIds` — when that exact combo is already staged the CTA flips to **Remove** (`stageCombo` toggles by key; a blind Add would silently unstage).
