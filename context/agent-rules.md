...
15. **Every new screen MUST be registered in the web deep-link config in `app/App.tsx`.** Adding a `<Stack.Screen>` is not enough. The `linking.config.screens` object in [`App.tsx`](../app/App.tsx) must list the new screen under its stack with a path prefixed by `` `${BASE}/...` `` (where `BASE = 'PindejosBowling'`, the GitHub Pages project base). The screen-name key must match the `<Stack.Screen name=...>` exactly.

    **Why this is mandatory:** React Navigation does not auto-prefix unlisted screens. Any screen missing from this config gets a generated, prefix-less URL from its name (e.g. `/PinsinoHelp`), which on the deployed site resolves to `<user>.github.io/PinsinoHelp` — *outside* the `/PindejosBowling/` project base — so it loads on first navigation but returns a hard GitHub "There isn't a GitHub Pages site here" 404 on refresh or when the URL is shared. Listing it with the `${BASE}` prefix keeps the URL under the project base, where the SPA `404.html` fallback (`deploy.yml`) can serve it. Keep the per-stack `initialRouteName` intact so the back button returns to the stack home instead of falling out to the first tab. See [page-creation.md](page-creation.md) §5 Step 4.
