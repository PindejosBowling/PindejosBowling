...
## Recurring broadcasts — weekly scheduled pushes (migration `20260720120000`)

Admin-defined **weekly push slots** (the driving case: RSVP nags that only hit players who haven't answered yet). One row per slot in `recurring_broadcast_schedules`: `audience` + `day_of_week` (0=Sunday..6=Saturday — matches `EXTRACT(DOW)` and JS `Date.getDay()`) + `send_time`/`timezone` (default America/New_York) + per-slot `category_id`, `route_key` (broadcastTargets wire key, NULL = opens app), static `title`/`body`, `enabled`, `last_fired_at`. Admin-only RLS (`FOR ALL is_admin()`, same as `broadcast_event_rules`).

- **Delivery reuses everything**: `materialize_due_recurring_broadcasts()` (SECURITY DEFINER, zero grants) runs at the **top of `invoke_broadcast_sender()`** each minute (own exception guard — a reminder bug can never block normal sending); a due slot INSERTs a normal `broadcasts` row (`source='recurring'`, `created_by` NULL, `scheduled_for=now()`, `data={route?, recurring_schedule_id, week_id?}`) which the **same tick** dispatches. No Edge Function or cron changes; opt-outs still enforced by `broadcast_recipients` at send.
- **Audiences** (the extension point — adding one = extend the CHECK + one branch in the materializer + a row in the screen's `AUDIENCES` map): `rsvp_non_responders` — targets = active players with **no `rsvp` row** (in *or* out both count as answered) for the current week (`getCurrent` rule); skips silently (no broadcasts row) when no current week or everyone answered. `everyone` — `target_player_ids NULL` (whole category).
- **Firing math**: most recent occurrence of (day, time) computed in local wall-clock space then converted once (DST-safe). Fires when `occurrence > last_fired_at`; **`last_fired_at` is stamped before any skip** (a skipped occurrence is consumed, never fired late), with a **2h lateness cap** so downtime never causes a middle-of-the-night nag. `last_fired_at` defaults to `now()` at creation (only future occurrences fire) and a BEFORE UPDATE trigger resets it on reschedule / re-enable (no surprise re-fires). The app never writes it.
- **Admin UI**: `BroadcastAdminScreen` → collapsible "RECURRING" section (slot rows + Switch, tap → `RecurringEditModal`: audience/day/time (ET wall clock)/category/tap-destination/copy, two-tap delete). History rows show a **RECURRING** badge. Data path: `db.ts recurringBroadcastSchedules` (`list`/`upsert`/`setEnabled`/`remove`) via `useBroadcastAdminData.rawRecurringSchedules`.
