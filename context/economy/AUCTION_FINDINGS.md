...
- **Auctions are week-agnostic** (schema review 2026-06-12): no `auctions.week_id` — their settlement clock is `closes_at`, not the archive tick. Auction ledger pairs carry **`week_id = NULL`**, which keeps them out of `archive_week`/`unarchive_week`'s week-scoped predicates by construction; `season_id` (required — balance and conservation scope per player+season) keeps them in the sums.
