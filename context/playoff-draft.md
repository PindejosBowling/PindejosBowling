...
- **Admin fixes**: direct table CRUD under the `admin can write` RLS policies
  (start draft = status flip, pool pruning, reset = delete draft → cascades).
  Status-coupled mutations get RPCs: `playoff_undo_pick` (also reopens `completed` →
  `drafting`), `playoff_materialize_teams`.
- **Materialize**: writes `teams` (numbered by seed) + `team_slots` (captain slot 1,
  picks in pick order) onto the playoff week; refuses if the week already has teams.
  From there the existing week-editor/matchups/scoring flow takes over — playoff code
  never touches `games`.
- **Reset caveat**: deleting a draft does not un-flag `weeks.is_playoff`.
