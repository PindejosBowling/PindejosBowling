...
| `PvPShootoutBackdrop` | none | PvP's art is a **full-viewport hero field** (the loudest rung, `sceneHero`): a midnight Texas shootout — bezel-to-ground procedural starfield (thinned over the central column, generous negative space) with a shooting star, one bright four-point star, and two buzzards circling above the duel; the shootout (gunslinger pins, holster glints, tumbleweed, saguaro, red eyes) pins to the bottom of the viewport. Mounted as the first child inside the SafeAreaView; measures itself via onLayout. |
| `SportsbookMenuBoardBackdrop` | none | The Sportsbook's art is a **scroll-length marquee "menu board"**, not a fixed scene: a bulb-studded gold/charcoal frame whose header panel is sized (`HEADER_PANEL_PX`) to wrap the title + view pills, with side rails running the full scroll length and a closing rail at the bottom — the betting lines are the menu, so the interior stays empty. A red eye pair hides in the bottom padding. Mounted as the first child **inside the ScrollView**; measures itself via onLayout. |
| `LoanSharkDepthBackdrop` | none | Loan Shark's art is a **scrolling full-bleed depth field**, not a fixed scene: a procedural pixel field the full length of the scroll content (measured via onLayout) telling a depth story that tracks loan risk top-to-bottom — solid beach (pebbles, abandoned dice, footprints walking into the surf) → surf → shallows → mid-water fins → deep → abyss with red eyes. Speckle density is full in the side gutters and sporadic behind the cards. Mounted as the first child **inside the ScrollView** so it scrolls with the page. |
