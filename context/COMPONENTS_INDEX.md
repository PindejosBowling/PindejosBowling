...
| `LedgerRow` | `{ entry: LedgerEntry, perspective: 'player'\|'house', isLast }` | The single renderer for both pin-ledger Activity surfaces. Derives an action label from `(type, perspective)` for every ledger type (bet/score/bonus/loan/pvp/bounty); bets from a special headline its snapshotted title (gold for `special`); bet-backed rows are tappable and open `BetDetailModal` internally. |
