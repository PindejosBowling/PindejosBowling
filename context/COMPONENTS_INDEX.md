...
| `BetDetailModal` | `{ bet: BetView \| null, onClose }` | Canonical single-bet breakdown overlay (bettor/season/week, consolidated legs with settled scores color-coded, wager/status/return). Renders nothing when `bet` is null. **Also exports the helpers** `resultBadge(status)`, `betPayout`, `betReturn`, `betReturnDisplay`, `betReturnText(bet, perspective)` used by every BetRow caller. |
