...
| [context/push-broadcasts.md](context/push-broadcasts.md) | **Push Broadcasts** — real iOS push notifications: the admin composer (send-now / scheduled / targeted), user opt-out (master + per-category; **opt-out always wins**, enforced server-side via the single `broadcast_recipients` predicate), the `send-broadcasts` Edge Function, the pg_cron+pg_net tick, Vault secrets, the **automated event-driven pushes** (Market Moves → push coupling: `broadcast_event_rules` + the feed trigger + admin-editable templates), and the **recurring weekly schedules** (`recurring_broadcast_schedules` + audience-resolving materializer — RSVP non-responder nags). Read before touching any `broadcast_*` / `push_*` / `recurring_*` code. NOT the in-app badge framework (that's notifications.md). |
