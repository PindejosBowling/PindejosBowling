...
| [context/push-broadcasts.md](context/push-broadcasts.md) | **Push Broadcasts** — real iOS push notifications: the admin composer (send-now / scheduled / targeted), user opt-out (master + per-category; **opt-out always wins**, enforced server-side via the single `broadcast_recipients` predicate), the `send-broadcasts` Edge Function, the pg_cron+pg_net tick, Vault secrets, and the recipe for adding an event-driven push later. Read before touching any `broadcast_*` / `push_*` code. NOT the in-app badge framework (that's notifications.md). |
