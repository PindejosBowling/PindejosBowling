...
- **Two scene shapes:**
  - *Anchored scenes* — small fixed compositions (a wanted poster top-right, a gavel bottom-right, a skyline along the bottom). One iconic silhouette per screen domain. (Historical shape: every shipped screen has since graduated to a procedural field, and the fixed-scene renderer was retired — reach for a field first.)
...
| Market Moves | Town square celebration (full-viewport field, the liveliest scene in the app): false-front storefront row, dark except the telegraph office — every window lit under a zigzag lime marquee; wires swooping in from both screen edges, sparking gold; fireworks bursting down the sky; confetti falling everywhere; a mixed-color crowd packed at the door, arms up, some mid-jump, hats flung overhead; a crier on a crate; a bird keeping its seat on the wire; red eyes in a dark upstairs window next door | The wire's come in and the news is BIG — the square goes off like a jackpot, except whoever bet the other way |
| PvP | High-noon duel (full-viewport hero field): the whole scene dropped to the bottom — two bowling-pin bandits (small head, the two classic neck stripes, bulging belly, narrow base, cowboy hat, a six-gun holstered on the hip with a gold glint, hand hovering ready to draw) facing off at the far left and right across a dusty street, saguaro, tumbleweed, a dark doorway with red eyes in the shade — under a wide, thin sky carrying only the blazing overhead sun and two circling buzzards | High noon — the two pins squared off, hands over their holsters, and someone in the shade waiting to see who draws first |
...
| Auction House | Auction floor as a banner across the bottom third (everything above left empty so it never competes with the cards): an auctioneer raised on a rostrum (right) with his gavel thrown up mid-call, a glowing lot on a tall pedestal (left) standing clear above the heads, and a deep packed mixed-color crowd shooting numbered bid paddles (gold + lime) up at jittered heights, a little floor dust in the air — red eyes watching from the shadows at the back | The gavel's about to drop and the whole floor is bidding at once — somebody in the dark already knows who wins |
...
- **Construction:** compose scenes by stamping small named sprites onto a blank canvas (`stamp`/`rekey` in [scenes.ts](scenes.ts)); generate atmosphere (stars, dust, water, dunes) procedurally with the deterministic per-cell hash so patterns are stable across renders. Cell size for fields: `FIELD_PIXEL` (8px).
