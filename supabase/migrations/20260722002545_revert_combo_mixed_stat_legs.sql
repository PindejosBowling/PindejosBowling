-- Revert mixed-stat combo legs (owner decision 2026-07-22: combining
-- different stat kinds into one summed value has awkward implications —
-- pins + spares don't belong on one number). Combos go back to ONE stat
-- across ≥2 distinct players.
--
-- `…000344_combo_mixed_stat_legs` is already applied on prod, so this
-- migration restores the pre-mixed behavior forward:
--   • compose_combo_bet, settle_week, preview_settle_week — restored verbatim
--     to their pre-mixed definitions (uniform member_ids × stat; the inline
--     c'''/preview combo computations return).
--   • combo_seed_line(uuid[], text, uuid, int) — restored to the direct
--     single-half-point implementation (…234333), no legs delegation.
--   • combo_seed_line(jsonb, uuid, int) and combo_market_status DROPPED.
-- Verified before writing this migration: zero combo markets carry
-- params.legs or a 'mixed|…' combo_key, so nothing depends on the dropped
-- paths. The KEPT single-half-point line math is unaffected.

DROP FUNCTION IF EXISTS public.combo_market_status(uuid, integer, jsonb);
DROP FUNCTION IF EXISTS public.combo_seed_line(jsonb, uuid, integer);
