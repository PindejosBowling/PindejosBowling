-- ============================================================================
-- PvP Challenge Contracts — corrective migration.
-- ============================================================================
-- Fixes four defects found reviewing the initial PvP implementation against the
-- live schema and the pin-economy conservation invariant:
--
--   1. create_pvp_challenge referenced v_week.lock_at — the weeks table has no
--      lock_at column (only bowled_at / is_confirmed / is_archived). COALESCE
--      evaluates it unconditionally, so EVERY create call errored at runtime
--      ("record v_week has no field lock_at"). Default the expiry to the week's
--      bowled_at instead (the de-facto lock point).
--
--   2. create_pvp_challenge's prop_duel market lookup selected `s.key` with no
--      `s` table in the FROM clause ("missing FROM-clause entry for table s"),
--      breaking every prop_duel creation. The aliased column was dead anyway.
--
--   3. settle_pvp_challenge's winner path wrote a standalone house-only
--      +rake_amount pvp_rake row with no offsetting row. The pot was already
--      fully escrowed into the house at accept, so the house keeps the rake as
--      the residual of (stakes − payout); the extra +rake row mints pins from
--      nothing and breaks the conservation invariant
--      (SUM(pin_ledger.amount) = SUM(score_credit), which includes house rows).
--      Fixed to a balanced pair: pay the winner the FULL pot, then collect the
--      rake back from the winner to the house. Net winner = payout_amount; net
--      house over the contract = +rake_amount; every row is part of a
--      player+house pair that nets to 0.
--
--   4. void_pvp_challenge claimed to reverse a settled contract's payout/rake
--      but only refunded stakes, so voiding a settled contract double-paid the
--      winner. Now it reverses every payout/rake movement before refunding.
--
-- Also hardens decline_pvp_challenge: a stranger could decline (cancel) an
-- open-board challenge they were not party to. Declines are now restricted to
-- the creator or the (set) counterparty.
-- ============================================================================


-- ============================================================================
-- 1. create_pvp_challenge — fix lock_at default + prop market lookup.
-- ============================================================================
...
-- ============================================================================
-- 2. decline_pvp_challenge — restrict to actual parties (no open-board grief).
-- ============================================================================
...
-- ============================================================================
-- 3. void_pvp_challenge — reverse payout/rake on settled contracts before refund.
-- ============================================================================
-- Refunds both stakes. For an already-settled contract it first reverses every
-- payout and rake movement (player + house pairs) so the winner is not paid
-- twice, then refunds the stakes. All reversals are balanced pairs.
...
BEGIN
  ...
  -- If already settled, reverse every payout/rake movement first (player +
  -- house pair, opposite sign of the original). pvp_ledger rows for these
  -- carry the player on the affected side; house side is mirrored.
  IF v_challenge.status = 'settled' THEN
    FOR v_row IN
      SELECT * FROM public.pvp_ledger
      WHERE challenge_id = p_challenge_id
        AND type IN ('payout', 'rake')
...
-- ============================================================================
-- 4. settle_pvp_challenge — conservation-correct rake on the winner path.
-- ============================================================================
-- The pot is already fully escrowed into the house at accept. The winner is paid
-- the FULL pot back (winner +total_pot / house -total_pot), then the rake is
-- collected from the winner to the house (winner -rake / house +rake). Net
-- winner = payout_amount; net house over the contract = +rake_amount; every pin
-- movement is a balanced player+house pair, so SUM(pin_ledger.amount) is
-- unchanged by PvP (conservation invariant holds). This replaces the prior
-- standalone house-only +rake row that minted pins from nothing.
...
BEGIN
  ...
  -- Winner path: pay the full pot to the winner, then collect rake back to house.
  -- Both are balanced player+house pairs (conservation-safe).
  -- Payout pair: winner +total_pot, house -total_pot.
  INSERT INTO public.pin_ledger (player_id, season_id, week_id, is_house, amount, type, description)
  ...
  -- Rake pair: winner -rake, house +rake (house keeps its cut).
  IF v_challenge.rake_amount > 0 THEN
    INSERT INTO public.pin_ledger (player_id, season_id, week_id, is_house, amount, type, description)
      VALUES (v_winner_id, v_challenge.season_id, v_challenge.week_id,
              false, -v_challenge.rake_amount, 'pvp_rake', 'PvP challenge rake')
      RETURNING id INTO v_pin_player;
    INSERT INTO public.pin_ledger (player_id, season_id, week_id, is_house, amount, type, description)
      VALUES (NULL, v_challenge.season_id, v_challenge.week_id,
              true, v_challenge.rake_amount, 'pvp_rake', 'PvP challenge rake (house)')
      RETURNING id INTO v_pin_house;

    INSERT INTO public.pvp_ledger (challenge_id, player_id, season_id, week_id, amount, type, description, pin_ledger_id)
      VALUES (p_challenge_id, v_winner_id, v_challenge.season_id, v_challenge.week_id,
              -v_challenge.rake_amount, 'rake', 'House rake', v_pin_player)
      RETURNING id INTO v_pvp_id;
