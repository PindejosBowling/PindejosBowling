...
> `compose_combo_bet` specs accept `"line"`. Accounting is untouched — legs
> still snapshot `odds_at_placement`/`line_at_placement` and payouts remain
> `floor(stake × Π odds)`. Full reference:
> [../context/odds-engine.md](../context/odds-engine.md).
>
> **Value-first lines (2026-07-22, same day).** Bettors pick the VALUE they
> intend to beat: `market_price_line`/`combo_price_line` quote ANY half-point
> line (posted rungs verbatim, unposted fresh inside the `custom_odds_min/max`
> band; never exposing the model), and placement mints on demand —
> `place_bet_at_lines` / quoted `compose_combo_bet` specs route through
> `bet_mint_rung_internal` (posted rung reused; absent → over/under PAIR
> minted at the fresh zero-vig price; quote drift beyond `quote_tolerance`
> rejects with `ODDS_MOVED|<market_id>|<quoted>|<fresh>`), then the untouched
> `place_house_bet` core. ⚰️ `combo_preview_ladder` deprecated (kept one
> release for deployed clients).
...
| `compose_combo_bet(week_id, combos jsonb, stake, extra_selection_ids?, insurance_item_id?, crutch_item_id?, boost_item_id?, extra_picks? jsonb)` → jsonb | **Player.** The bet slip's combo placement path — `combos` = an ARRAY of specs `{member_ids, stat, scope, game_number?, line?, quoted_odds?}` (≥1). A quoted spec's unposted chosen line MINTS via `bet_mint_rung_internal` (fresh AND dedup paths); without `quoted_odds`, legacy posted-rungs-only behavior. `extra_picks` = line-shaped regular legs `[{market_id, line, quoted_odds}]` riding the same ticket (minted the same way; self-referential markets rejected). Per spec: validate (week unlocked; stat/scope/schedule game; ≥2 distinct members all RSVP'd `'in'`), dedup on `params.combo_key` (identical live combo → join it; the same combo twice on one ticket RAISEs), else create the `combo` market + selections seeded by `combo_seed_line`. Then ONE bet via `place_house_bet(all overs ∥ extras, stake, items…)` — **compose = bet, atomic** (no unbet combo market can exist; a ticket parlays combos with single lines AND other combos; self-referential extras rejected; item ids pass through). One week-level advisory xact lock serializes composes. Publishes at most ONE `sportsbook_combo_composed` per bet (first created combo + `combo_count`). Returns `{bet_id, combos: [{market_id, line, deduped}]}`. `authenticated`. [context/combo-lines.md](../context/combo-lines.md). |
| `market_price_line(market_id, line? default NULL)` → jsonb | **Player, STABLE.** Value-first preview: quote ANY half-point line on one priceable market (`over_under`/`prop`/`combo`; open OR closed — the client gates staging). Posted rungs echo posted odds verbatim (frozen markets never re-quoted); unposted lines price fresh via `odds_engine_market_distribution` + `odds_engine_price_pair` inside the custom band (`custom_odds_min/max`, NULL → `odds_min/max`). NULL line = the seed rung. Returns `{line, odds, posted, seed_line, seed_odds, min_line, max_line}` — the band's HIGH edge is the stat's physical cap (counts 9.5/game, night 10·g−0.5; score O/U 220/game — game 220.5, night 220·g+0.5, combos 220/game/member); the LOW edge is the **offer floor**, the smallest line paying `odds_min` (×1.20). Every half-point inside prices at the **fair zero-vig odds, no ceiling** (long tails pay true model price). `odds` null = line unavailable (below the floor / outside the caps); the distribution itself never leaves the server. `authenticated`. |
| `combo_price_line(member_ids[], stat, season_id, n_games?, week_id?, game_number?, line?)` → jsonb | **Player, STABLE.** The same quote for a combo member set being composed: an existing open market (same `combo_key`) → posted rungs verbatim + its seed anchors; no market → `combo_seed_line` anchor, summed member distributions (an unposted SEED force-prices clamped, mirroring the minter). Same return shape. `authenticated`. |
| `bet_mint_rung_internal(market_id, line, quoted_odds)` → uuid | **Internal (no grants).** One rung's authoritative price-and-mint: validates open + priceable + half-point (`trim_scale` normalization so `4.50`/`4.5` share one `over:<line>` key); posted rung → tolerance-check the quote (`quote_tolerance`) and reuse; absent → price fresh at the fair unclamped odds (out-of-range RAISEs "not available") and INSERT the over/under **pair at the fresh price** (client odds never stored; unders must exist for grading + PvP counterparty). Race-safe: `ON CONFLICT (market_id, key) DO NOTHING` + re-read + re-check. Drift → `RAISE 'ODDS_MOVED|<market_id>|<quoted>|<fresh>'` (machine-parseable — the app's confirm sheet). `sort_order = 100 + 2·line` (after the ladder's 0..13). |
| `place_bet_at_lines(picks jsonb, stake, insurance_item_id?, crutch_item_id?, boost_item_id?)` → uuid | **Player.** The value-first placement wrapper: `picks` = `[{market_id, line, quoted_odds}]` (distinct markets); each leg through `bet_mint_rung_internal`, then the untouched `place_house_bet` core — atomic, so a failed placement rolls minted rungs back (**no betless custom rung can persist**). `authenticated`. |
| `combo_seed_line(member_ids[], stat, season_id, n_games default 1)` | STABLE helper → the seeded line for an arbitrary member set (`team_prop_seed_line` generalized to `unnest`). **Projection-anchored** (`…190000_seed_lines_from_projection`): engine on → Σ per-member `floor(projected mean × n_games)` + **one** half point, clamped; engine off → the legacy averages: Σ per-member `floor(avg × n_games)` + one half point, clamped (frame stats off official-import averages, `total_pins` off `player_raw_avg_score`). Since `…234333_fix_combo_seed_line_single_half_point` — the original `floor(Σavg × n)+0.5` accumulated per-member fractions and overstated the line vs. the members' displayed solo lines. `authenticated` (combine-mode live preview). |
...
| `lanetalk_seed_lines(player_id)` | STABLE helper → the player's legacy average seeds (count lines `floor(avg)+0.5` clamped, raw clean-frames-per-game — the sync scales it by the week's schedule — first-ball `round(avg,1)`) from their official imports; zero rows when no history. Since `…190000_seed_lines_from_projection` the sync uses these only as the engine-off fallback + the no-history eligibility gate — live seeds anchor on the engine mean (`odds_engine_seed_line`). `authenticated`. |
...
> - **Mispriced lines.** The seed line is `floor(engine mean)+0.5` (projection-
>   anchored since `…190000_seed_lines_from_projection`; previously the raw
>   average) — still an estimate of a skewed, non-stationary distribution. A leg
>   whose true `P(win)` ≠ 0.5 is mildly +EV as a single but its edge compounds
>   **exponentially** in a parlay.
...
| `migrations/20260722205136_value_line_pricing.sql` | **Value-first lines 1/2** — on-demand pricing: `odds_engine_config` gains `custom_odds_min/max` + `quote_tolerance`; `odds_engine_norm_ppf` (Acklam inverse CDF, band edges in closed form); `odds_engine_market_distribution` (one market → the generators' exact distribution, internal); `market_price_line` + `combo_price_line` (client-granted quotes, never exposing mean/variance) over shared `odds_engine_quote_internal`. |
| `migrations/20260722210417_value_line_seed_force.sql` | **Value-first follow-up** — the SEED line always quotes: an unposted seed (fresh combo) force-prices clamped (mirroring the minter) and the selectable band widens to contain the seed. |
| `migrations/20260722210732_value_line_placement.sql` | **Value-first lines 2/2** — mint-on-demand placement: `bet_mint_rung_internal` (posted-rung reuse w/ tolerance, fresh over/under pair mint at the fresh price, `ODDS_MOVED\|…` contract, ON CONFLICT race safety), `place_bet_at_lines` (line-shaped picks → `place_house_bet`), `compose_combo_bet` re-signed (`quoted_odds` specs mint unposted lines on both paths; `p_extra_picks` line-shaped parlay legs). |
