...
| `bet_mint_rung_internal(market_id, line, quoted_odds)` → uuid | **Internal (no grants).** One rung's authoritative price-and-mint: validates open + priceable + half-point (`trim_scale` normalization so `4.50`/`4.5` share one `over:<line>` key); posted rung → tolerance-check the quote (`quote_tolerance`) and reuse; absent → price fresh in the custom band (out-of-band RAISEs "not available") and INSERT the over/under **pair at the fresh price** (client odds never stored; unders must exist for grading + PvP counterparty). Race-safe: `ON CONFLICT (market_id, key) DO NOTHING` + re-read + re-check. Drift → `RAISE 'ODDS_MOVED|<market_id>|<quoted>|<fresh>'` (machine-parseable — the app's confirm sheet). `sort_order = 100 + 2·line` (after the ladder's 0..13). |
