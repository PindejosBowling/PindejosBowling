...
| `lanetalk_seed_lines(player_id)` | STABLE helper → the player's four seeded lines (counts `floor(avg)+0.5` clamped, clean% `floor(avg/5)*5+2.5`, first-ball `round(avg,1)`) from their official imports; zero rows when no history. `authenticated`. |
