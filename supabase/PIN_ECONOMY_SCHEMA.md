...
|---|---|
| `id` uuid PK | |
| `market_type` text | `over_under` \| `moneyline` \| `prop` \| `team_prop`. Discriminator. Extend the CHECK to add types. |
...
| **Team-aggregate prop (LIVE)** | `bet_markets(market_type=team_prop, subject_game_id, game_number, params={family:'team_aggregate', stat:'total_pins'\|'clean_frames'\|'strikes'\|'spares', scope:'game', team_id, team_number, clock:'archive'\|'lanetalk'})` + over/under selections sharing a `line` (even-money) — graded by the shared `settle_market_internal` engine. Anchored like moneyline (`subject_game_id` + `params.team_id`; `subject_player_id` null). Auto-generated per (game × team × stat) by `sync_team_prop_markets_for_week` (coupled via `resync_week_markets`); line seeded by `team_prop_seed_line` (Σ roster per-game averages, floored to a half-point once). **Two settlement clocks:** `total_pins` (`clock='archive'`) settles at archive inside `settle_betting_for_week` (Σ `scores` of the team for the game — the moneyline aggregation); the frame stats (`clock='lanetalk'`) settle next-day on the LaneTalk clock and are **exempted from the archive backstop** alongside `market_type='prop'`. Anti-tank: `prevent_self_tank` blocks under-back/over-lay by players rostered (non-fill) on `params.team_id`. |
